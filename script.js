...
// Supabase istemcisini burada tanımlıyoruz, ancak kullanımı DOMContentLoaded içinde olacak
let supabase; // supabase client'ını burada tanımlıyoruz, değeri daha sonra atanacak
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Supabase istemcisini burada oluşturuyoruz, bu nokta itibarıyla supabase kütüphanesi yüklenmiş olmalı
    try {
        ...
        if(seekBar) { // Null check just in case
             seekBar.addEventListener('input', seek); // Seek when the user drags the seek bar
        }
        ...
        async function renderMusics() {
            ...
            if (!supabase) {
                console.error("Supabase istemcisi henüz hazır değil.");
             ...
             console.log("renderMusics çalışıyor..."); // Debug log
            ...
            try {
                ...
                if (error) {
                    ...
                    const errorMessage = '<p class="text-red-400 text-center mt-4">Müzikler yüklenemedi: ' + error.message + '</p>'; // Display error message
                    if (musicListDesktop) musicListDesktop.innerHTML = errorMessage;
                    if (musicListMobile) musicListMobile.innerHTML = errorMessage;
                    updatePlayerUIState();
                    return;
        ...
        */
        ...
        // renderMusics(); // <-- Bu satır DOMContentLoaded içinde çalışacak

    } catch (error) {
        console.error("DOMContentLoaded içinde hata:", error);
        // Eğer supabase.createClient hatası burada olursa yakalanır
        alert("Uygulama başlatılırken bir hata oluştu. Konsolu kontrol edin.");
    }
});

// renderMusics çağrısı artık DOMContentLoaded listener'ı içindedir.
// Diğer fonksiyonlar (addMusic, deleteMusic, signIn, signOut vb.) de
// DOMContentLoaded içinde tanımlandığı ve kullanıldığı için supabase objesine erişebilir.

// Mobile menu button listener (HTML'de varsa)
